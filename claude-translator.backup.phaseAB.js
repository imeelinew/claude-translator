...
(function (window, document, undefined) {
    ...
    function transTextNode(node) {
        ...
        // 跳过忽略区域内的节点（IGNORE_SELECTORS 为空时跳过检查，避免 closest('') 报错）
        if (IGNORE_SELECTORS && node.parentElement?.closest(IGNORE_SELECTORS)) return;
    ...
    function transAttr(el, attr) {
        if (!el) return;
        if (IGNORE_SELECTORS && (el.matches?.(IGNORE_SELECTORS) || el.closest?.(IGNORE_SELECTORS))) return;
    ...
    function transInlineSentence(el) {
        ...
        if (!raw || !/should Claude consider in responses\?/i.test(raw)) return;

        const link = el.querySelector('a');
        if (!link) return;

        const linkText = (link.textContent || '').trim();
        if (!/personal preferences/i.test(linkText) && linkText !== '个人偏好') return;
    ...
    function traverseNode(root) {
        ...
        const walker = document.createTreeWalker(
            ...
            {
                acceptNode(node) {
                    // 跳过忽略区域整个子树（IGNORE_SELECTORS 为空时跳过检查，避免 matches('') 报错）
                    if (IGNORE_SELECTORS &&
                        node.nodeType === Node.ELEMENT_NODE &&
                        node.matches?.(IGNORE_SELECTORS)) {
